    ...
    (set, get) => ({
      ...
      addEntry: async (count: number) => {
        ...
        try {
          ...
          const { error } = await supabase.from('pushups').insert(newEntry);

          if (error) throw error;
          console.log(
            `[Pushup Store] Successfully created entry ${newEntry.id}`
          );
        } catch (error) {
          console.warn(
            `[Pushup Store] Failed to sync entry ${newEntry.id}:`,
            error
          );
        ...
        for (const operation of pendingOperations) {
          ...
          try {
            if (operation.retryCount >= MAX_RETRY_ATTEMPTS) {
              console.error(
                `[Pushup Store] Max retries exceeded for operation on ${operation.id}`
              );
              continue;
            }
      ...
      importEntries: async (entries: PushupEntry[]) => {
        ...
        try {
          ...
          const { error } = await supabase.from('pushups').upsert(entries);
        ...
        try {
          ...
          if (serverEntries) {
            ...
            serverEntries.forEach((serverEntry) => {
              ...
              if (
                !localEntry ||
                new Date(serverEntry.updated_at) >
                  new Date(localEntry.updated_at)
              ) {
                set((state) => {
                  const newEntries = new Map(state.entries);
      ...
      getTotalForDate: (date: Date) => {
        ...
        return Array.from(get().entries.values())
          .filter((entry) => {
            ...
            return (
              (entryDate.isAfter(startOfDay) ||
                entryDate.isSame(startOfDay, 'second')) &&
              (entryDate.isBefore(endOfDay) ||
                entryDate.isSame(endOfDay, 'second'))
            );
          })
      ...
      getStreak: () => {
        ...
        while (true) {
          const hasEntryForDay = entries.some(
            (entry) =>
