...
### Recent Performance & Refactoring Updates

1. **Performance Analysis & Logging**:

   - ✅ Added `performance.now()` metrics throughout store actions (`addCompletion`, `toggleHabitStatus`, `calculateAndUpdate`, `updateAffectedDates`) to measure execution time.
   - ✅ Added performance logging in `HabitDetailsSheet` to track time from click to render.
   - ✅ Output performance metrics to console for analysis.

2. **`calculateDateStatus` Optimization**:

   - ✅ Refactored `calculateDateStatus` utility (now in `lib/utils/habits.ts`) for significant performance improvement.
   - ✅ Uses single-pass filtering and efficient Map lookup for completion statuses.

3. **Store Subscription Optimization**:

   - ✅ Wrapped `calculateAndUpdate` and `updateAffectedDates` calls within the store subscription in `setTimeout` to defer heavy calculations and prevent blocking the main thread.

4. **Store Structure Refactoring**:

   - ✅ Split `SharedSlice` type into modular slice interfaces (`HabitSlice`, `CompletionSlice`, etc.) colocated with their respective action files (`lib/stores/habits/actions/*.ts`).

5. **Utility File Reorganization**:
   - ✅ Moved habit utilities to `lib/utils/habits.ts`.
   - ✅ Moved achievement utilities to `lib/utils/achievements.ts`.
